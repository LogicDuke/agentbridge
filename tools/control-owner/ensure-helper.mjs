/*
 * Explicit provisioning gate for the Decision 062 / PR #84 native control artifacts.
 *
 * This gate is a prerequisite of the CONTROL-ORIENTED operator flow ONLY
 * (`npm run control`: TypeScript build -> this gate -> control CLI, and the explicit
 * `npm run control:provision`). It is deliberately NOT wired into `npm run cockpit`
 * or `npm run cockpit:live`: helper provisioning must never be a prerequisite for
 * read-only Cockpit availability (CONTROL_PROVISION_FAILURE does not imply
 * COCKPIT_FAILURE). The live runtime attempts the control channel only after the
 * Cockpit is serving, and any control fault there fails closed while the Cockpit
 * stays up and read-only.
 *
 * The explicit control launch must not silently omit the mandatory native artifacts
 * and their generated provenance. There are TWO, each with its own provenance and
 ...
 * creator's CREATOR_* equivalents) and the control channel is unavailable. The
 * control script runs this orchestrator between the TypeScript build and the Node
 * launch so a clean — or a partially/torn-provisioned — checkout is made coherent
 * before control-anchor verification runs; a provisioning failure is loud and
 * nonzero.
 *
 * The idempotent skip is taken ONLY when EVERY artifact and its provenance form the
 * CANONICAL pair for the CURRENT reviewed source (validateHelperPair /
 * validateCreatorPair in helper-pair.mjs): the on-disk provenance bytes must equal
 * that artifact's `encode(sha256(binary bytes), sourceId(reviewed source))` exactly.
 * Existence — or fields found somewhere in the text — is NOT sufficient: an
 * interrupted `build.mjs` can leave a new executable beside stale provenance (a torn
 * pair), a truncated/duplicated/augmented provenance module, or a self-consistent
 * pair built from an OLDER reviewed source. Any representation not emitted verbatim
 * by the shared encoder for the current source triggers a rebuild through the
 * existing trusted builder, after which every pair is validated again; if any is
 * still not canonical the launch fails loud/closed. That is the provisioning
 * invariant: SUPPORTED_PROVISIONING_SUCCESS implies NATIVE_ARTIFACT_RUNTIME_COMPATIBLE,
 * because the only artifact accepted is one built from the exact source the runtime
 ...
 * already-trusted `build.mjs`, which alone compiles the reviewed C sources and emits
 * each provenance from the exact binary bytes via the SAME canonical encoders. It
 * uses no shell — the one external program (the same Node) is invoked by absolute
 * path with an explicit argv (execFileSync, shell:false). Provenance is compared as
 * bytes against the encoder output; it is never imported or JS-parsed, so a
...
function main() {
  if (process.platform !== 'win32') {
    // The native artifacts build only on Windows and the control channel is a
    // Windows feature; on other platforms it degrades closed. Skipping keeps
    // cross-platform launches (e.g. the read-only cockpit on Linux/macOS) working
    // without MSVC.
    note(`non-Windows platform (${process.platform}); native artifacts not required, skipping.`);
    process.exit(0);
  }

  const before = firstInvalidPair();
  if (before === null) {
    // Already provisioned with VALID, source-current pairs: do not recompile on
    // every launch. The runtime still hashes each binary against its generated
...
// (`npm run control` / `control:provision`), never imported; the reusable validators
// live in helper-pair.mjs. A guard comparing import.meta.url with the argv[1] file
