 ...
 * lifecycle regression can import and exercise the REAL canonical encoders and
 * validators (not copies). The module (helper-pair.mjs) lives under tools/ and is
...
/**
 * Decide whether the owner-helper/provenance pair on disk is the canonical pair for
 * the CURRENT reviewed source — on-disk provenance bytes equal
 ...
 * its OWN reviewed source. A cross-wired provenance module is never valid.
 */
...
/**
 * The canonical owner-helper provenance encoder (re-exported): helper SHA-256 +
 * reviewed-source SHA-256 (both lowercase 64-hex) -> exact provenance module text.
 * Pure; throws a TypeError on a malformed digest.
 */
export declare function encodeProvenance(sha256Hex: string, sourceId: string): string;

/**
 * The canonical descriptor-creator provenance encoder (re-exported): creator SHA-256
