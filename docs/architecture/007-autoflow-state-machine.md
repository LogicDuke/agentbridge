...
- `INVOCATION_REPORTED` checks binding **after** identity, because the commit it
...
- `HUMAN_GATE_OPENED` checks status posture **before** its payload, so an
  already-open gate returns `HUMAN_GATE_ALREADY_OPEN` without `atCommitSha` being
  read at all. A request to open a gate that is already open is answered by the
  gate, never by the shape of the payload accompanying it.
